...
class DoubleMathTest extends FunSuite {
  ...
  test("Double functions") {
    ...
    assert(isnegzero(sqrt(-0d)))
    assert(isposzero(sqrt(0d)))
    ...
    assert(isneginf(inversesqrt(-0d)))
    assert(isposinf(inversesqrt(0d)))
    ...
    assert(isnan(min(nan, 2d)))
    assert(2 == (min(posinf, 2d)))
    assert(isneginf(min(neginf, 2d)))
    ...
    assert(isnan(max(nan, 2d)))
    assert(isposinf(max(posinf, 2d)))
    assert(2 == (max(neginf, 2d)))
    ...
    assert(1 == clamp(1d, 1d, 3d))
    assert(2 == clamp(2d, 1d, 3d))
    assert(3 == clamp(3d, 1d, 3d))
