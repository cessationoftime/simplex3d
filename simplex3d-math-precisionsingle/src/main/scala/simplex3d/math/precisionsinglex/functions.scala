...
object functions {
  ...
  // Constants
  /** Machine float epsilon, pow(2, -23).
   */
  final val Epsilon = 1.1920928955078125e-7f
   ...
   */
  def modf(u: inVec2f, i: Vec2f) :Vec2f = {
    i.x = trunc(u.x)
    i.y = trunc(u.y)
    u - i
  }
  ...
  def modf(u: inVec3f, i: Vec3f) :Vec3f = {
    i.x = trunc(u.x)
    i.y = trunc(u.y)
    i.z = trunc(u.z)
    u - i
  ...
  def modf(u: inVec4f, i: Vec4f) :Vec4f = {
    i.x = trunc(u.x)
    i.y = trunc(u.y)
    i.z = trunc(u.z)
    i.w = trunc(u.w)
  ...
  def outerProduct(c: inVec2f, r: inVec3f) :Mat3x2f = {
    new Mat3x2f(
      c.x*r.x, c.y*r.x,
      c.x*r.y, c.y*r.y,
      c.x*r.z, c.y*r.z
  ...
  def outerProduct(c: inVec2f, r: inVec4f) :Mat4x2f = {
    new Mat4x2f(
      c.x*r.x, c.y*r.x,
      c.x*r.y, c.y*r.y,
      c.x*r.z, c.y*r.z,
  ...
  def outerProduct(c: inVec3f, r: inVec2f) :Mat2x3f = {
    new Mat2x3f(
  ...
  def outerProduct(c: inVec3f, r: inVec4f) :Mat4x3f = {
    new Mat4x3f(
      c.x*r.x, c.y*r.x, c.z*r.x,
      c.x*r.y, c.y*r.y, c.z*r.y,
      c.x*r.z, c.y*r.z, c.z*r.z,
  ...
  def outerProduct(c: inVec4f, r: inVec2f) :Mat2x4f = {
    new Mat2x4f(
      c.x*r.x, c.y*r.x, c.z*r.x, c.w*r.x,
      c.x*r.y, c.y*r.y, c.z*r.y, c.w*r.y
    )
  }
  def outerProduct(c: inVec4f, r: inVec3f) :Mat3x4f = {
    new Mat3x4f(
      c.x*r.x, c.y*r.x, c.z*r.x, c.w*r.x,
      c.x*r.y, c.y*r.y, c.z*r.y, c.w*r.y,
  ...
  def determinant(m: inMat2f) :Float = m.m00*m.m11 - m.m10*m.m01
   ...
   * <br/>If matrix determinant is zero the result is undefined.
   */
  def inverse(m: inMat3x2f) :Mat3x2f = {
    import m._
   ...
   * <br/>If matrix determinant is zero the result is undefined.
   */
  def inverse(m: inMat4x3f) :Mat4x3f = {
    import m._
   ...
   */
  def angleAxis(q: inQuat4f, axis: Vec3f) :Float = {
    import q._
   ...
   */
  def angleAxis(m: inMat3f, axis: Vec3f) :Float = {
    import m._
