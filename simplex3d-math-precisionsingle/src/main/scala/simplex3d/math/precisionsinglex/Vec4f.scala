...
with Protected with Serializable
{
  ...
  final def /(s: Float) = { val inv = 1/s; new Vec4f(x * inv, y * inv, z * inv, w * inv) }
  ...
  final def *(m: inMat2x4f) :Vec2f = m.transposeMult(this)
  final def *(m: inMat3x4f) :Vec3f = m.transposeMult(this)
  ...
  final override def hashCode :Int = {
    ...
    ) + simplex3d.math.floatHashCode(w)
  }
...
object ConstVec4f {
  ...
  def apply(m: AnyMat2[_]) = new ConstVec4f(m.f00, m.f01, m.f10, m.f11)
...
object Vec4f {
  ...
  final val Tag = classTag[Vec4f]
  final val ConstTag = classTag[ConstVec4f]
  final val ReadTag = classTag[ReadVec4f]
  ...
  def apply(m: AnyMat2[_]) = new Vec4f(m.f00, m.f01, m.f10, m.f11)
